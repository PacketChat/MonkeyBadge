...
def test_register_success():
    r = requests.post("http://localhost:8000/register", json={"handle": handle, "key": registration_key, "myUUID": thisbadge})
    json_data = json.loads(r.text)
    assert json_data['UUID'] == thisbadge, "UUIDs do not match"

def test_register_bad_key(): 
    r = requests.post("http://localhost:8000/register", json={"handle": handle, "key": "badkey", "myUUID": thisbadge})    
...
def test_register_bad_uuid():
    r = requests.post("http://localhost:8000/register", json={"handle": handle, "key": registration_key, "myUUID": "baduuid"})
    assert r.status_code == 400, "Status code is not 400"
